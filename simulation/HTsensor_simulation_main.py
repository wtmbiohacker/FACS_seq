...
#number of mutants in initial library
if 'LibSize' not in config or config['LibSize']=='':
    LibSize=50
    print('the default value of library size is 50!')
else:
    LibSize=int(config['LibSize'])
...
print ('python HTsensor_simulation_generateLib.py --lib_size %d --exp_con %s --bin_bou_con %s --distribution %s --readth %s --output_prefix %s 2>error.log'%(LibSize, experiment_configure,    Bin_boundary_configure,log10u_sigma_dis,str(ReadsThreshold),outputname))
os.system('python HTsensor_simulation_generateLib.py --lib_size %d --exp_con %s --bin_bou_con %s --distribution %s --readth %s --output_prefix %s 2>error.log'%(LibSize, experiment_configure,Bin_boundary_configure,str(log10u_sigma_dis),str(ReadsThreshold),outputname))
...
os.system('rm *.pickle bin_occupation_configure.txt cell_count_configure.txt')
os.system('mv %s %s_results/%s_library/'%(my_lib, prefix, prefix))
